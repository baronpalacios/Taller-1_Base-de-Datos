...
-- +++++++++++++++++++++ESTUDIANTES++++++++++++++++++++++++++
-- **********************************************************
-- PALACIOS MOSQUERA HARLINTON
-- CRUZ BUILES EDUARD ADRIAN
-- **********************************************************
...
-- ============================================================
CREATE TABLE area (
    cod_area     VARCHAR(20)  NOT NULL PRIMARY KEY,
    nombre_area  VARCHAR(100) NOT NULL UNIQUE
);

-- ============================================================
--  -- SE CREA TABLA PERSONA  (superclase ISA)
-- ============================================================
...
-- ============================================================
--  -- SE CREA TABLA EMPLEADO  (subclase de PERSONA)
-- ============================================================
...
CREATE INDEX idx_emp_jefe ON empleado(jefe_id);

-- ============================================================
--  -- SE CREA TABLA  (subclase de EMPLEADO)
-- ============================================================
CREATE TABLE medico (
    identificacion  VARCHAR(20)  NOT NULL PRIMARY KEY,
...
CREATE TABLE enfermera (
    ...
    tipo              VARCHAR(20) NOT NULL CHECK (tipo IN ('auxiliar','asistente','jefe')),
...
CREATE INDEX idx_enf_area ON enfermera(cod_area);

-- ============================================================
---- SE CREA TABLA HABILIDAD_ENFERMERA - atributo multivaluado DE LA TABLA ENFERMERA
-- ============================================================
...
-- ============================================================
-- -- SE CREA TABLA  (subclase de PERSONA)
-- ============================================================
...
CREATE TABLE cama (
    ...
    cod_area  VARCHAR(20)  NOT NULL,
    estado    VARCHAR(20) NOT NULL CHECK (estado IN ('libre','ocupada')) DEFAULT 'libre',
...
CREATE INDEX idx_cama_area ON cama(cod_area);

-- ============================================================
-- -- SE CREA TABLA HISTORIA_CLINICA (entidad débil de PACIENTE)
-- ============================================================
...
CREATE INDEX idx_cons_fecha ON consulta(fecha_consulta);
CREATE INDEX idx_cons_medico ON consulta(id_medico);
...
CREATE TABLE cita (
    ...
    motivo         VARCHAR(300),
    estado         VARCHAR(20) NOT NULL CHECK (estado IN ('pendiente','confirmada','cancelada','completada')) DEFAULT 'pendiente',
    notas          TEXT,
...
-- ============================================================
--  -- SE CREA TABLA ASIGNACION_CAMA, la cual se crea por la relación de N:M de Cama y paciente
-- ============================================================
...
CREATE INDEX idx_asig_cama ON asignacion_cama(nro_cama, cod_area);

-- ============================================================
--  -- SE CREA TABLA CAMPANIA
-- ============================================================
CREATE TABLE campania (
    cod_campania      VARCHAR(20)  NOT NULL PRIMARY KEY,
...
CREATE INDEX idx_camp_medico ON campania(id_medico_resp);
CREATE INDEX idx_camp_fecha ON campania(fecha_realizacion);

-- ============================================================
--  -- SE CREA TABLA PARTICIPACION_CAMPANIA, la cual se general por la relaciónde N:M de Pacientes y Campaña
-- ============================================================
...
-- ============================================================
--  Se agrega la tabla USUARIOS, la cual almacena los credenciales de acceso al sistema
-- ============================================================
...
CREATE TABLE usuarios (
    ...
    rol           VARCHAR(20) NOT NULL CHECK (rol IN ('admin','medico','recepcionista')) DEFAULT 'recepcionista',
...
SELECT
    ...
    EXTRACT(YEAR FROM age(pa.fecha_nacimiento))::INT AS edad,
...
INSERT INTO medico VALUES
    ('EMP001', 'Pediatría',        'LIC-00123', 'Universidad del Valle', TRUE),
    ('EMP002', 'Medicina General', 'LIC-00456', 'Universidad Javeriana', TRUE),
    ('EMP003', 'Cirugía General',  'LIC-00789', 'Universidad Nacional',  TRUE);
...
INSERT INTO cita (id_paciente, id_medico, fecha_cita, hora_cita, motivo, estado, notas, created_at) VALUES
    ('PAC001', 'EMP001', CURRENT_DATE, '09:00:00', 'Control rutinario', 'confirmada', NULL, CURRENT_TIMESTAMP),
    ('PAC002', 'EMP002', CURRENT_DATE, '10:30:00', 'Chequeo general', 'pendiente', NULL, CURRENT_TIMESTAMP),
    ('PAC003', 'EMP003', CURRENT_DATE + INTERVAL '1 day', '08:00:00', 'Retiro de yeso', 'pendiente', NULL, CURRENT_TIMESTAMP);
